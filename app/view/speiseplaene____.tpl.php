    ...
    <table border="1" class="speiseplan_tbl">
        ...
        <tr>
            ...
            for ($i = 1; $i <= 5; $i++) {
                ...
                <td colspan="2">
                    <div style="border-bottom: 1px solid #ccc;">Speise 1</div>
                        ...
                        if ($bestellung->getId() && count($bestellung_has_speiseVerwaltung->findeAnhandVonBestellungId($bestellung->getId())) > 0) {
                            ...
                            foreach ($bestellung_has_speisen as $bestellung_has_speise) {
                                if ($bestellung_has_speise->getSpeiseNr() > $sp_nr_check) {
                                    ...
                                    ?>
                                </div><div style="border-bottom: 1px solid #ccc;">Speise 2</div><div style="height:125px;">
                                    <?php
        ...
        foreach ($portionen_tage as $projekt_id => $portionen_tag_kid) {
            ...
            foreach ($portionen_tag_kid as $sbar_kunden_id => $portionen_tag_menunr) {
                ...
                foreach ($portionen_tag_menunr as $speise_nr => $portionen_tag) {
                    ...
                    echo '<pre>';
                    // var_dump($projekt_id, $sbar_kunden_id, $speise_nr, $portionenaenderung_vorhanden);
                    echo '</pre>';
					...
					//var_dump($check_kunde->getKitafinoGruppen());
                    if ($check_kunde->getKitafinoGruppen()) {
                        $portionenaenderungVerwaltung->speichere($neue_portionenaenderung);
                    }
                    echo '<pre>';
                   // var_dump($projekt_id, $sbar_kunden_id, $speise_nr, $neue_portionenaenderung, $portionenaenderung_vorhanden);
                    echo '</pre>';
                }
            }
        }
...
// Abrechnungstage ergänzen
            ...
            /*
              ...
              $check_tag = $abrechnungstagVerwaltung->findeAnhandVonKundeIdUndTag($kunde->getId(), $tag_ts_check);

              if (!$check_tag->getId()) {
              $best_check = $bestellungVerwaltung->findeAnhandVonTag($tag_ts_check);
              $best_to_check_auf_speisen = $bestellung_has_speiseVerwaltung->findeAnhandVonBestellungId($best_check->getId());
              var_dump($best_to_check_auf_speisen[0]);
              if($best_to_check_auf_speisen[0]->getSpeiseId()) {
              $abrechnungstag_check = $abrechnungstagVerwaltung->findeAnhandVonKundeIdUndTag($kunde->getId(), $tag_ts_check);
              echo $abrechnungstag_check->getKundeId();
              }
            ...
            /*
              foreach($tage_ts_woche_array as $tage_ts_tag) {
              $best_check = $bestellungVerwaltung->findeAnhandVonTag($tage_ts_tag);
              $bestellungen_ids_check[] = $best_check->getId();
              }
              var_dump($bestellungen_ids_check);

              foreach($bestellungen_ids_check as $best_id_check) {
              $best_to_check = $bestellung_has_speiseVerwaltung->findeAnhandVonBestellungId($best_id_check);
              var_dump(count($best_to_check));
              }
             */
// Abrechnungstage ergänzen Ende
            ...
            <?php if ($_REQUEST['dev'] == 2) { ?>
                ...
                foreach ($tage_ts_woche_array as $tag_ts) {
                    ...
                    <td>
                        ...
                        if ($kunde->getKundennummer()) {
                            //checke ob schon über kitafino
                            //$kitafino_infos_kunde = checkeKitafinoStarttermin($kunde->getKundennummer());

                            $starttermin = $kitafino_infos_kunde['starttermin'];
                            $heute_ts = mktime(0, 0, 0, date('m', $tag_ts), date('d', $tag_ts), date('Y', $tag_ts));
                            /* secho '<pre>';
                              var_dump($starttermin, $heute_ts);
                              echo '</pre>'; */
                            if ($starttermin) {
                                if ($starttermin > $heute_ts) {
                                    $gestartet = false;
                                }
                            }
                        }
                        ...
                        if ($kunde->getKundennummer() && $tag_nr == 0) {

                            $fehler_meldung = '';



                            //checke kitafino settings auf fehler
                            //  $kitafino_gruppen_setting = $kunde->getKitafinoGruppen();
                            $sbar_kunden_mit_pid = $kundeVerwaltung->findeAlleMitKundennummer($kunde->getKundennummer());
                            $kitafino_gruppen = findeGruppenAusKitafino($kunde->getKundennummer());

                            if ($kitafino_gruppen == 'Fehler: Keine korrekte Kitafino-ID') {
                                echo '<span>Fehler: Keine korrekte Kitafino-ID</span>';
                            }
                            if ($kitafino_gruppen_setting == NULL && count($kitafino_gruppen) == 1 && count($sbar_kunden_mit_pid) == 1) {
                                $show_gruppen = false;
                                $kunde->setKitafinoGruppen($kitafino_gruppen[0]);
                            }
                            if (count($sbar_kunden_mit_pid) == 1 && count($kitafino_gruppen) > 1) {
                                $fehler_meldung .= 'Anzahl Kunden zu PID: ' . count($sbar_kunden_mit_pid) . '<br />Anzahl Gruppen in kitafino: ' . count($kitafino_gruppen) . '<br />';
                                foreach ($kitafino_gruppen as $gruppe) {
                                    $gruppen_arr[] = $gruppe['id'];
                                }
                                $kunde->setKitafinoGruppen(implode(',', $gruppen_arr));
                                // var_dump(implode(',',$gruppen_arr));
                            }
                            if (count($sbar_kunden_mit_pid) > count($kitafino_gruppen)) {
                                $fehler_meldung .= 'Anzahl Kunden zu PID: ' . count($sbar_kunden_mit_pid) . '<br />Anzahl Gruppen in kitafino: ' . count($kitafino_gruppen) . '<br />';
                                $fehler_meldung .= 'Achtung! Diese ID ist in der S-Bar ' . count($sbar_kunden_mit_pid) . ' mal vorhanden.<br /> Aber nur ' . count($kitafino_gruppen) . ' Gruppe(n) in kitafino:';
                                foreach ($kitafino_gruppen as $grp) {
                                    $fehler_meldung .= '<br/>' . $grp['bezeichnung'];
                                }
                            }

                            if (count($sbar_kunden_mit_pid) == count($kitafino_gruppen) && count($sbar_kunden_mit_pid) > 1 && $kitafino_gruppen_setting == NULL) {
                                $fehler_meldung .= 'Die Gruppen wurden noch nicht zugewiesen.';
                            }

                            echo '<div style="color: red;">' . $fehler_meldung . '</div>';
                        }
                        ?>
                        <?php
                        // kitafino CONNECT
                        if ($_REQUEST['dev']) {
                            ...
                            if ($kunde->getKundennummer()) {
                                ...
                                $kitafino_gruppen_ids = explode(',', $kunde->getKitafinoGruppen());

                                $kitafino_gruppen_ids = array_filter($kitafino_gruppen_ids);
                                // include 'functions/connect_db.php';
                                //  $count_orders = ermittleZahlenZuKundeTag($tage_ts_woche_array[$tag_nr], $projekt_id_db, $kitafino_gruppen_ids);
                                $count_orders = $count_orders_tage[$tag_nr];
                                ?>
                                <br/>
                                <img style="margin-top:5px;" src="images/kitafino_icon.png" /> <strong><?php echo $projekt_id_db ?>
                                </strong>
                                <?php
                                if ($gestartet == false) {
                                    echo '<span style="font-size:12px;">kitafino ab ' . strftime('%d.%m.%Y', $starttermin) . '</span>';
                                }
                                ?>

                                <?php
                                //s-bar zahlen ersetzen
                                //  $portionen_array[$tag_nr] = $count_orders['Speise 1']['gesamt'];
                            }
                        }
                        ?>
                    </td>
                    ...
                    <td>
                        <input style="float:left; clear:none; " class="portionen_input" type="text" name="portionen_<?php echo $tage_strs[$tag_nr] ?>[]" value="<?php echo $portionen_array[$tag_nr] ?>" onblur="javascript: checkAnzahlPortionen(this)" onkeyup="javascript: checkAnzahlPortionen(this)"/>
                        <?php
                        if ($portionen_array[$tag_nr] != $standardportionen_array[$tag_nr]) {
                            echo '<sup>1</sup> (' . $standardportionen_array[$tag_nr] . ')';
                        }
                        $small_error = '';
                        if ($portionen_array[$tag_nr] != $count_orders['Speise 1']['gesamt']) {
                            $alert_kitafino_zahlen = 'border: 2px dotted red;';
                            $small_error = '<span style="color: red;font-size: 11px;">S-Bar ≠ kitafino</span>';
                        }
                        ?>

                        <?php if ($kunde->getKundennummer() && $gestartet) { ?>

                            <input class="portionen_input" style="clear:none;float: left; background: url(images/kitafino_icon.png) no-repeat  left center ;background-size: 30%; padding-left: 10px; border:1px solid #999; <?php echo $alert_kitafino_zahlen ?> " type="" disabled="disabled"  value="<?php echo $count_orders['Speise 1']['gesamt'] ?>" />

                            <br/>
                            <?php echo $small_error ?>
                            <?php
                            if ($show_gruppen) {
                                foreach ($count_orders['Speise 1'] as $gruppenname_key => $orders_gruppe) {
                                    if ($gruppenname_key == 'gesamt') {
                                        continue;
                                    }
                                    if ($orders_gruppe == 0 && $gruppenname_key != 'gesamt') {
                                        continue;
                                    }
                                    ?>   
                                    <label style="float: left; clear: none;"><?php echo $gruppenname_key ?></label>
                                    <input style="float: left; clear: none; background: url(images/kitafino_icon.png) no-repeat  left center ;background-size: 30%; padding-left: 10px; border:none; " type="" disabled="disabled" class="portionen_input" value="<?php echo $orders_gruppe ?>" />
                ...
                <?php if ($kunde->getAnzahlSpeisen() > 1) {
                    ?>
                    <tr class="<?php echo $highlight_class ?>">
                        <?php
                        $tag_nr = 0;
                        foreach ($tage_ts_woche_array as $tag_ts) {
                            ...
                            <td>
                                ...
                                <?php echo $kunde->getName(); ?> <?php if ($kunde->getAnzahlSpeisen() > 1) { ?> - Speise 2<?php } ?>
                                <?php echo $echo_mo ?>
                                <?php
                                // kitafino CONNECT
                                if ($_REQUEST['dev']) {
                                    ...
                                    if ($kunde->getKundennummer()) {
                                        $projekt_id_db = $kunde->getKundennummer();
                                        // include 'functions/connect_db.php';
                                        // $count_orders = ermittleZahlenZuKundeTag($tage_ts_woche_array[$tag_nr], $projekt_id_db);
                                        $count_orders = $bestellzahlen[$tag_nr];
                                        ?>
                                        <br/>
                                ...
                                //s-bar zahlen ersetzen
                                //  $portionen_array[$tag_nr] = $count_orders['Speise 2']['gesamt'];
                                ?>
                            </td>
                            <td>
                                <input class="portionen_input" style="float: left; clear: none;" type="text" name="portionen2_<?php echo $tage_strs[$tag_nr] ?>[<?php echo $rowcount - 2 ?>]" value="<?php echo $portionen2_array[$tag_nr] ?>" onblur="javascript: checkAnzahlPortionen(this)" onkeyup="javascript: checkAnzahlPortionen(this)"/>

                                <?php
                                if ($kunde->getKundennummer()) {
                                    $small_error = '';
                                    if ($portionen2_array[$tag_nr] != $count_orders['Speise 2']['gesamt']) {
                                        $alert_kitafino_zahlen = 'border: 2px dotted red;';
                                        $small_error = '<span style="color: red;font-size: 11px;">S-Bar ≠ kitafino</span>';
                                    }
                                    ?>

                                    <input style="float: left; clear: none; background: url(images/kitafino_icon.png) no-repeat  left center ;background-size: 30%; padding-left: 10px; border:none; <?php echo $alert_kitafino_zahlen ?> " type="" disabled="disabled" class="portionen_input" value="<?php echo $count_orders['Speise 2']['gesamt'] ?>" />

                                    <br/>
                                    <?php echo $small_error ?>
                                    <?php
                                    foreach ($count_orders['Speise 2'] as $gruppenname_key => $orders_gruppe) {
                                        if ($gruppenname_key == 'gesamt') {
                                            continue;
                                        }
                                        ...
                                        ?>   
                                        <label style="float: left;"><?php echo $gruppenname_key ?></label>
                                        <input style="float: left; clear: none; background: url(images/kitafino_icon.png) no-repeat  left center ;background-size: 30%; padding-left: 10px; border:none;" type="" disabled="disabled" class="portionen_input" value="<?php echo $orders_gruppe ?>" />
                                        <br />                                   
                                    <?php } ?>
                                <?php } ?>
            ...
            <?php } else { ?>
                ...
                <tr class="<?php echo $highlight_class ?>">
                    ...
                    <td>
                        <input style="<?php echo $alert_kitafino_zahlen ?>" class="portionen_input" type="text" name="portionen_di[]" value="<?php echo $portionen_di ?>" onblur="javascript: checkAnzahlPortionen(this)" onkeyup="javascript: checkAnzahlPortionen(this)"/>
                        <?php
                    ...
                    <td>
                        <input style="<?php echo $alert_kitafino_zahlen ?>" class="portionen_input" type="text" name="portionen_mi[]" value="<?php echo $portionen_mi ?>" onblur="javascript: checkAnzahlPortionen(this)" onkeyup="javascript: checkAnzahlPortionen(this)"/>
                        <?php
                    ...
                    <td>
                        <input style="<?php echo $alert_kitafino_zahlen ?>" class="portionen_input" type="text" name="portionen_do[]" value="<?php echo $portionen_do ?>" onblur="javascript: checkAnzahlPortionen(this)"  onkeyup="javascript: checkAnzahlPortionen(this)"/>
                        <?php
                    ...
                    <td>
                        <input style="<?php echo $alert_kitafino_zahlen ?>" class="portionen_input" type="text" name="portionen_fr[]" value="<?php echo $portionen_fr ?>" onblur="javascript: checkAnzahlPortionen(this)" onkeyup="javascript: checkAnzahlPortionen(this)"/>
                        <?php
                ...
                <?php if ($kunde->getAnzahlSpeisen() > 1) { ?>
                    <tr  class="<?php echo $highlight_class ?>">
                        ...
                        <td style="border-bottom: 0px solid #888;">
                            ...
                            <input class="portionen_input" type="text" name="portionen2_mo[<?php echo $rowcount - 2 ?>]" value="<?php echo $portionen2_mo ?>" onblur="javascript: checkAnzahlPortionen(this)" onkeyup="javascript: checkAnzahlPortionen(this)"/>
                            <?php
                        ...
                        <td style="border-bottom: 0px solid #888;">
                            ...
                            <input class="portionen_input" type="text" name="portionen2_di[<?php echo $rowcount - 2 ?>]" value="<?php echo $portionen2_di ?>" onblur="javascript: checkAnzahlPortionen(this)" onkeyup="javascript: checkAnzahlPortionen(this)"/>
                            <?php
                        ...
                        <td style="border-bottom: 0px solid #888;">
                            ...
                            <input class="portionen_input" type="text" name="portionen2_mi[<?php echo $rowcount - 2 ?>]" value="<?php echo $portionen2_mi ?>" onblur="javascript: checkAnzahlPortionen(this)" onkeyup="javascript: checkAnzahlPortionen(this)"/>
                            <?php
                        ...
                        <td style="border-bottom: 0px solid #888;">
                            ...
                            <input class="portionen_input" type="text" name="portionen2_do[<?php echo $rowcount - 2 ?>]" value="<?php echo $portionen2_do ?>" onblur="javascript: checkAnzahlPortionen(this)"  onkeyup="javascript: checkAnzahlPortionen(this)"/>
                            <?php
                        ...
                        <td style="border-bottom: 0px solid #888;">
                            ...
                            <input class="portionen_input" type="text" name="portionen2_fr[<?php echo $rowcount - 2 ?>]" value="<?php echo $portionen2_fr ?>" onblur="javascript: checkAnzahlPortionen(this)" onkeyup="javascript: checkAnzahlPortionen(this)"/>
                            <?php
