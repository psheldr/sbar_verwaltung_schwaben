...
<!--
    <div class="loader_cover2" style="text-align:center;position:fixed;top: 30px;right: 30px;">
        <div>
            <h4>Wird geladen...</h4>
            <img src="images/load.gif"  />
        </div>
    </div>-->
<div class="loader_cover" style="position:fixed;top:0;left: 0;width:100%;padding:500px;z-index:10000; background-color: rgba(200,200,200,0.8)">
...
if ($_REQUEST['dev1']) {
	...
	echo '<pre>';
			...
			$check_arr = array();
foreach ($kunden_kitafino as $kd) {
	$check_arr[] = $kd->getKundennummer();
}
...
sort($check_arr);
        ...
        $refresh_page = false;
        if ($_REQUEST['do'] == 'import_kitafino' /* || $_REQUEST['dev2'] */) { //TEMP disabled für manuelle eingabe
            $refresh_page = true;
            $time_start = microtime(true);
            $kunden_kitafino = $kundeVerwaltung->findeAlleMitKundennummerKitafino();
            //$kunden_kitafino = $kundeVerwaltung->findeAlleAuchInaktiveMitKundennummerKitafino();
...
///////////////
            ...
            $import_ct = count($bestellzahlen);
            $kw = date('W', $ts_starttag_der_woche);
            $logname = 'last_kf_import';
            $log = 'KW'.$kw.';'.date("d.m.Y H:i:s") . ';' . $laufzeit . ';Zahlen für ' . $import_ct . ' S-Bar Kunden importiert' . PHP_EOL;
            file_put_contents('/is/htdocs/wp1055935_1F55KTNHIC/www/sbar/verwaltung_schwaben/logs/'.$logname.'.log', $log, FILE_APPEND);  /**/
...
// Abrechnungstage ergänzen Ende
            ...
            if ($portionenaenderung->getId()) {
                ...
                $daten_neue_port_aenderung['portionen_mo'] = $portionen_mo;
                $daten_neue_port_aenderung['portionen_di'] = $portionen_di;
                $daten_neue_port_aenderung['portionen_mi'] = $portionen_mi;
                $daten_neue_port_aenderung['portionen_do'] = $portionen_do;
                $daten_neue_port_aenderung['portionen_fr'] = $portionen_fr;
            ...
            if ($portionenaenderung2->getId()) {
                ...
                $daten_neue_port_aenderung['portionen_mo'] = $portionen2_mo;
                $daten_neue_port_aenderung['portionen_di'] = $portionen2_di;
                $daten_neue_port_aenderung['portionen_mi'] = $portionen2_mi;
                $daten_neue_port_aenderung['portionen_do'] = $portionen2_do;
                $daten_neue_port_aenderung['portionen_fr'] = $portionen2_fr;
            ...
            if ($portionenaenderung3->getId()) {
                ...
                $daten_neue_port_aenderung['portionen_mo'] = $portionen3_mo;
                $daten_neue_port_aenderung['portionen_di'] = $portionen3_di;
                $daten_neue_port_aenderung['portionen_mi'] = $portionen3_mi;
                $daten_neue_port_aenderung['portionen_do'] = $portionen3_do;
                $daten_neue_port_aenderung['portionen_fr'] = $portionen3_fr;
            ...
            if ($portionenaenderung4->getId()) {
                ...
                $daten_neue_port_aenderung['portionen_mo'] = $portionen4_mo;
                $daten_neue_port_aenderung['portionen_di'] = $portionen4_di;
                $daten_neue_port_aenderung['portionen_mi'] = $portionen4_mi;
                $daten_neue_port_aenderung['portionen_do'] = $portionen4_do;
                $daten_neue_port_aenderung['portionen_fr'] = $portionen4_fr;
...
<?php if ($refresh_page) { ?>
    ...
    <script type="text/javascript">
        ...
        window.location.replace("https://www.s-bar.net/verwaltung_schwaben/index.php?action=cockpit&show_import_info=1&cat=kitafino&starttag=<?php echo $start_tag_woche2 ?>&startmonat=<?php echo $start_monat_woche2 ?>&startjahr=<?php echo $start_jahr_woche2 ?>&kids=<?php echo $kitafino_ids ?>");
